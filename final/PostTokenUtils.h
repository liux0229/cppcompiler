...
template<> EFundamentalType FundamentalTypeOf<std::nullptr_t>() { return FT_NULLPTR_T; }
...
void printToken(const PPToken& token) {
  if (token.type == PPTokenType::Eof) {
    ...
    std::cout << format("{} {} {}", token.typeName(), encoded.size(), encoded) 
              << std::endl;
  }
}
...
std::string getSuffix(const PPToken& token, int quote)
{
  if (!token.isUserDefined()) {
    ...
    auto it = std::find(token.data.rbegin(), 
                   token.data.rend(),
                   quote);
    CHECK(it != token.data.rend());
    std::vector<int> suffix(it.base(), token.data.end());
    CHECK(!suffix.empty());
    std::string strSuffix = Utf8Encoder::encode(suffix);
    if (strSuffix[0] != '_') {
      Throw("ud_suffix `{}` does not begin with `_` from {}",
            strSuffix,
            token.dataStrU8());
    }
    return strSuffix;
